...
- **Backend Auto-Derived & Read-Only Venue Attributes (`familyNearby`)**: Certain venue properties (such as `familyNearby`) are auto-derived strictly by the backend (`web-jam-back`'s `isFamilyNearby()` via stored venue address and geo-distance). The backend's `stripReadOnlyFields()` unconditionally strips `familyNearby` from incoming `POST /venue` and `PATCH /venue/:id` payloads. Never make such auto-derived fields interactive/editable in the front-end or wire them to send client values in request payloads; keep the controls disabled / read-only with clear auto-derived labeling so users understand the value is computed from address/coordinates upon save.
