...
- **Venue Form Optional Enums & Selectable Attributes (`familyNearby`)**: In `EditVenueDialog`, optional enum fields (`templateOverride`, `audienceAttention`, `venueType`) must never be sent as empty strings (`""`) in request payloads, as backend Mongoose schema validation rejects empty strings against allowed enum values; omit or delete empty enum values before sending. The `familyNearby` checkbox is user-selectable in the UI and sent as a boolean in the payload.
