...
- **Venue Form Optional Enums & Derived-With-Override Attributes (`familyNearby`)**: In `EditVenueDialog`, optional enum fields (`templateOverride`, `audienceAttention`, `venueType`) must never be sent as empty strings (`""`) in request payloads, as backend Mongoose schema validation rejects empty strings against allowed enum values; omit or delete empty enum values before sending. `familyNearby` is a two-mode field: when the client sends no explicit value, the backend derives it from the stored venue address via `web-jam-back`'s `isFamilyNearby()` (geo-distance); when the client sends an explicit boolean, that value overrides the derived one and persists. The front-end therefore keeps the `familyNearby` checkbox interactive and sends it as a boolean in the payload. This depends on the companion widening of `stripReadOnlyFields()` in `web-jam-back` landing first — until that backend change deploys, `stripReadOnlyFields()` still strips `familyNearby` from every incoming `POST /venue` and `PATCH /venue/:id` body, so a user-set value is silently discarded and only the derived value is stored.
