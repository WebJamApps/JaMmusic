...
describe('SongEditor', () => {
  ...
  it('onChangeSongs runs setNewEditor', () => {
    ...
    onChangeSong(evt, { image: {}, gig: {}, song: { ...defaultSong, _id:'adls;kjf' }, isValid:true, hasChanged:true }, setNewEditor);
    expect(setNewEditor).toHaveBeenCalled();
  });
  it('SongFormTitle Edit', () => {
    const editor = { hasChanged:false, isValid:true, song: { ...defaultSong, _id:'adls;kjf' }, gig: {}, image: {} };
  ...
  it('SongFormTitle Create', () => {
    const editor = { song: defaultSong, gig: {}, image: {}, hasChanged:false, isValid:true };
    const songFormTitle = renderer.create(<SongFormTitle editor={editor} />).root;
  ...
  it('SongButtons runs addSongAPI', () => {
    ...
    const songButtons = renderer.create(<SongButtons editor={{ song, gig: {}, image: {}, isValid:true, hasChanged:true }}
  ...
  it('EditSongButtons click Cancel button', () => {
    ...
    const editSongButtons = renderer.create(<EditSongButtons setEditor={setNewEditor} setSongs={jest.fn()}
      auth={{} as Auth} editor={{ song, gig: {}, image: {}, isValid:true, hasChanged:true }} />).root;
    editSongButtons.findByProps({ id: 'cancel-edit-song' }).props.onClick();
    expect(setNewEditor).toHaveBeenCalled();
  });
  it('SongForm handleCategoryChange', () => {
    const editor = { isValid:true, hasChanged:true, song: { category: 'original', year: 2020, title: '', url: '' }, gig: {}, image: {} };
    const setNewEditor = jest.fn();
  ...
  it('handleCategoryChange', () => {
    ...
    handleCategoryChange(evt, { isValid:true, hasChanged:true, 
      song: { category: '', year: 0, title: '', url: '' }, gig: {}, image: {} }, setNewEditor);
    expect(setNewEditor).toHaveBeenCalled();
  });
